...
# How to use
Using the library is really simple. You just need to decorate your assembly with the ```UnionOf``` Attribute. This can be done in any file you want, as long as it precedes the namespace declaration of that file.  
Here an example:  
```csharp
...
namespace ExampleProject
{
	static public class Program
	{
		...
		{
			FloatingPointNumber floatingPointNumber1 = 42d;
...
```  
All you need to declare a tagged union is the ```[assembly: UnionOf()]``` attribute with at least 2 types passed to the constructor. The code generator will generate a name for you if you don't specify one (i.e. UnionOf_String_Int32).
...
## Operators
As you can see in the example above you can implicitly assign any of the union types to the generated type. Apart from that the generator is also going to generate overloads for the binary-or operator (```|```). So in case you are not sure whether or not the tagged union type that has been passed to your method was actually initialized you can assign i.e. default values like so:  
```csharp
...
[assembly: UnionOf(typeof(Half), typeof(float), typeof(double), Typename = "FloatingPointNumber")]
...
...
## Is-Method
The code generator will also generate n-overloads for the ```Is``` method, where n is the number of types the tagged union type is composed out of. This method will help you identify the actual value of the union type when required. It looks as close to pattern matching as possible with just the method signature. Unfortunately checking with the method is not exhaustive, meaning even if you have checked all but one type you will still need to call the method to get the actual value instead of just writing ```else```.
```csharp
...
