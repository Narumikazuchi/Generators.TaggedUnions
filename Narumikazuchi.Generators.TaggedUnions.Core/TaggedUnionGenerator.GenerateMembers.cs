...
public partial class TaggedUnionGenerator
{
                                          ...
                                          HashSet<ITypeSymbol> types)
    {
        ...
        foreach (ITypeSymbol type in types)
        {
            ...
            builder.AppendLine($"    public readonly bool Is([NotNullWhen(true)] out {typeString} result)");
            builder.AppendLine("    {");
                                       ...
                                       HashSet<ITypeSymbol> types)
    {
        ...
        {
            builder.AppendLine("        return m_Tag switch");
            builder.AppendLine("        {");
            for (Int32 index = 1;
                 index <= types.Count;
                 index++)
                                            ...
                                            HashSet<ITypeSymbol> types)
    {
        ...
        fields.Append($"    private readonly Tag m_Tag = Tag.Uninitialized;");
        tags.AppendLine("    }");
        tags.AppendLine();
